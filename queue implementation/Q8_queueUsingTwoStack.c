...
int dequeue(Queue *queue){
    ...
    Stack *temporaryStack = createStack();
    while(sizeOfStack(queue->stack) > 1)
        push(temporaryStack, pop(queue->stack));
    int valueToBeDeleted = pop(queue->stack);
    while(sizeOfStack(temporaryStack))
        push(queue->stack, pop(temporaryStack));
